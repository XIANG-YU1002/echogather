...
**Feature Branch**: `001-product-spec-consolidation`（spec 目錄識別名；本次未另建 git 分支，現行工作分支為 `chore/adopt-spec-kit`）
...
**Independent Test**: 任選 3 項已知差異（例如搜尋後端仍含活動、活動幣別欄位未實作、Admin 後端限制未完整強制），僅憑新文件判斷「規格要求什麼、目前實作到哪、差異狀態為何」，判斷結果與 CURRENT_DECISIONS 第 16 節一致。

**Acceptance Scenarios**:

1. **Given** 一項已確認但實作尚未同步的規格（例如活動層級幣別欄位），**When** 查閱新文件，**Then** 該規格以正式需求呈現，且已知差異章節明確標示目前實作狀況與「待同步」狀態。
2. **Given** 程式碼中存在但無決策紀錄的行為（例如保留中的角色改名端點），**When** 查閱新文件，**Then** 該行為不以正式需求形式出現，僅出現在已知差異的現況描述中。
...
3. 修改或刪除八份既有來源文件與 `docs/CURRENT_DECISIONS.md`（既有文件先保留原狀，不刪除、不覆寫）。
...
- **FR-009**: 已確認規格與目前實作狀況 MUST 分開呈現；「已知規格與實作差異」章節 MUST 完整收錄 CURRENT_DECISIONS 第 16 節全部 14 項差異（16.1–16.14，可重新編號但不可遺漏、不可淡化），並加入整理過程新發現的工程差異。
...
- **AC-004**: CURRENT_DECISIONS 第 16 節全部 14 項已知差異完整收錄且狀態標示正確。
...
- **AC-008**: 八份既有來源文件與 `docs/CURRENT_DECISIONS.md` 內容零變更（以 git 驗證）。
...
- **已知規格與實作差異項**：規格要求、目前實作狀況、狀態（待同步）三元組；基準為 CURRENT_DECISIONS 第 16 節 14 項，可增加新發現項。
...
- **SC-002**: 規格與實作零混淆——全部已知差異項（≥14 項）在新文件中的「規格／現況／狀態」三元組與 CURRENT_DECISIONS 第 16 節 100% 相符。
...
- `docs/CURRENT_DECISIONS.md` v1.1（2026-08-03）在本 Feature 進行期間為最新版本；若期間該文件更新，以更新後版本為準，並重新檢查受影響章節。
- CURRENT_DECISIONS 第 17 節目前為空（無待專案負責人確認的產品規則），故整理起點沒有懸而未決的產品裁決；整理過程新出現者依〈衝突處理規則〉辦理。
- 八份既有來源文件與 CURRENT_DECISIONS 在本 Feature 進行期間維持凍結，不會被其他工作同時修改。
