...
```text
official_price >= 0
official_currency ∈ {TWD, CNY, JPY, KRW, USD}
```

> 修訂紀錄：幣別原本固定 TWD，2026-07-24 擴充為五種可選（migration
> `0002_add_currency_values` 加入 KRW／USD）。

---

## 11.2b Character Creation Timing

> 新增紀錄（2026-07-30 使用者要求）。

管理員在商品表單新增角色時，**當下即建立角色**（寫入資料庫），
不是等商品送出時才連帶建立。

因此角色一經新增就實際存在，打錯字會留下垃圾資料。配套規則：

- 商品表單的角色標籤需同時提供「從此商品移除關聯」與「刪除角色」兩個動作。
- 「刪除角色」只對**本次新建**的角色開放，避免誤刪其他商品正在使用的既有角色。
- 已有商品關聯的角色一律不可刪（§12.5），此為後端強制，前端限制只是第一層。

---

## 11.2a Product Currency Consistency

> 新增紀錄（2026-07-30 使用者規則）。

**同一活動底下的商品，官方價幣別必須一致**：不能有的標台幣、有的標日圓。

- 只約束**有填官方價**的商品。沒填價格的商品 `official_currency` 為 NULL，
  不構成限制，也不會被此規則擋下。
- 建立與修改商品時都驗證。修改時**排除商品自己**——否則活動內唯一的標價商品
  會被自己的舊幣別鎖死，永遠改不了。
- 限制只在同一活動內；不同活動可以各自使用不同幣別。
- 違反時回 409 `ACTIVITY_CURRENCY_MISMATCH`，並在 details 回傳該活動目前的幣別，
  讓管理員知道要改成什麼。

在後端驗證而非只靠前端限制：同一活動的商品可能由不同管理員、不同時間建立，
只有後端擋得住不一致的資料。

前端配合：商品批次新增頁把幣別提到**活動層級**選一次、套用到整批；
活動已有標價商品時鎖定為既有幣別（見 docs/05 §29.1 的 `product_currency` 欄位）。
...
> 修訂（2026-07-30）：移除「修改角色名稱」。全站沒有安排任何適合改名的位置，
> 經使用者裁決取消該功能。後端 `PATCH /admin/characters/{id}` 保留但沒有前端
> 入口；名稱打錯的處理方式是刪除後重新新增（尚無商品關聯時可刪）。
...
## 20.8 Per-Character Max Quantity

有角色的商品，每個角色各有自己的接單上限，商品層級的 `max_quantity` 是各角色上限的總和（去正規化值，由後端加總後寫入）。

每角色上限允許 **0＝不接這個角色的單**：

```text
group_buy_product_character.max_quantity >= 0
group_buy_product.max_quantity          >  0
```

以下三種 0 不允許，由 API 層驗證（跨列條件，CHECK 約束無法表達）：

| 情況 | 錯誤碼 | HTTP |
| --- | --- | --- |
| 商品只有一個角色，該角色填 0 | `SINGLE_CHARACTER_QUANTITY_ZERO` | 422 |
| 多角色但全部填 0 | `ALL_CHARACTER_QUANTITIES_ZERO` | 422 |
| 該角色上限低於已占用數量 | `CHARACTER_MAX_QUANTITY_BELOW_OCCUPIED` | 409 |

前兩者的語意相同：整個商品都不接單時，應該取消勾選（或移除）該商品，而不是把上限填 0。第三者與 §20 商品層級的 `MAX_QUANTITY_BELOW_OCCUPIED` 是同一條規則，只是以角色為單位。

上限 0 不需要另外的狀態判斷：`available_quantity = max(上限 - 占用, 0)`（§20.2），可用量為 0 時即視為額滿，該角色在跟團介面自然呈現為不可選。

> 新增紀錄（2026-07-30）：依使用者需求開放。情境是單商品多角色，團主可依需求拒接特定角色（缺貨或不代購），不必把整個商品移出開團。同批也補上開團編輯頁的每角色上限輸入欄——在此之前每角色上限只能在建立開團時決定，開團後無法調整。
